---
character: zibai
role: "Главный ДД Лунного кристалла, от защиты"
updated: 2026-09-24
patch: "7.0"
weapons:
  - slug: lightbearing-moonshard
    note: "Лучший вариант"
  - slug: uraku-misugiri
  - slug: light-of-foliar-incision
  - slug: harbinger-of-dawn
    note: "R5, бюджетно"
  - slug: peak-patrol-song
artifacts:
  - sets: [night-of-the-sky-s-unveiling]
    note: "4 предмета — лучший вариант с большим отрывом"
  - sets: [husk-of-opulent-dreams]
    note: "4 предмета"
  - sets: [husk-of-opulent-dreams, night-of-the-sky-s-unveiling]
    note: "2 + 2"
mainStats:
  sands: "Защита %"
  goblet: "Защита %"
  circlet: "Шанс / Крит. урон"
substats: ["Шанс / Крит. урон", "Защита %", "Восст. энергии"]
teams:
  - name: "Лунный кристалл"
    members: [zibai, aino, gorou, linnea]
  - name: "Лунный кристалл с Коломбиной (Бездна 7.0)"
    members: [zibai, linnea, illuga, columbina]
sources:
  - title: genshin.gg
    url: https://genshin.gg/characters/zibai/
  - title: Game8
    url: https://game8.co/games/Genshin-Impact/archives/570263
videos:
  - id: "9jAJnci_-HM"
    author: "Miron MinMax"
    title: "Цзы Бай — очередная имба Нод-Края? Гайд в 6.3"
  - id: "PqhK68SZd3g"
    author: "Genshin Academy"
    title: "От простого к сложному — гайд на Цзы Бай"
external:
  - title: "KeqingMains Quick Guide"
    url: https://keqingmains.com/q/zibai-quickguide/
    author: KQM
    lang: en
---

**Суть:** Гео-ДД, весь урон которой масштабируется от защиты и идёт через Лунный кристалл (Гео + Гидро). Для её усиления нужны персонажи Лунного знамения: Коломбина, Иллуги или Айно.

**Статы:** защита в часах и гоблете, крит в короне. МС в доп. статах тоже полезно — оно усиливает Лунный кристалл.
